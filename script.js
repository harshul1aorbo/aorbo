...
function initializeCarousels() {
    ...
    carousels.forEach(carousel => {
        ...
        const navigation = carousel.nextElementSibling;
        ...
        const moveSlides = (direction) => {
            ...
            const offset = -currentIndex * 408; // 384px slide width + 24px gap
            track.style.transform = `translateX(${offset}px)`;
            updateButtonStates();
        };
        
        // Event listeners for buttons only
        ...
        track.addEventListener('touchstart', preventScroll, { passive: false });
        track.addEventListener('touchmove', preventScroll, { passive: false });
        track.addEventListener('touchend', preventScroll, { passive: false });
        track.addEventListener('wheel', preventScroll, { passive: false });
        track.addEventListener('mousewheel', preventScroll, { passive: false });
        track.addEventListener('DOMMouseScroll', preventScroll, { passive: false });
    });
}
